                                    ...
                                    <tbody>
                                        ...
                                        @else
                                            @foreach ($events as $event)
                                                ...
                                                @if ($event->status==3)
                                                    <td>
                                                        <i class="fa fa-lock text-danger" aria-hidden="true" title="Bu etkinlik yönetici tarafından askıya alınmıştır"></i>
                                                    </td>
                                                @elseif ($event->status==4)
                                                    <td>
                                                ...
                                                @else
                                                    @php
                                                        $updateUrl = route('user.event.update', ['id' => $event->id]);
                                                        $freezeUrl = route('user.event.freeze', ['id' => $event->id]);
                                                        $unfreezeUrl = route('user.event.unfreeze', ['id' => $event->id]);
                                                        $cancelUrl = route('user.event.cancel', ['id' => $event->id]);
                                                    @endphp
                                                    <td>
                                                        <a href="{{ $updateUrl }}" class="update"><i class="fa fa-pencil" aria-hidden="true" title="Düzenle"></i></a>
                                                        @if($event->finish_date > Carbon\Carbon::now())
                                                            <a href="{{ $freezeUrl }}" class="freeze @if($event->status!=1) hide @endif"><i class="fa fa-snowflake-o" aria-hidden="true" title="Dondur"></i></a>
                                                            <a href="{{ $unfreezeUrl }}" class="unfreeze @if($event->status!=2) hide @endif"><i class="fa fa-sun-o" aria-hidden="true" title="Dondurmayı kaldır"></i></a>
                                                            <a href="{{ $cancelUrl }}" class="cancel @if($event->status==3 || $event->status==4) hide @endif"><i class="fa fa-times" aria-hidden="true" title="İptal Et"></i></a>
                                                        @else
